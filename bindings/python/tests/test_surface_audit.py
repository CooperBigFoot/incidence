...
def test_complete_public_surface_has_no_bare_time_series() -> None:
    ...
    counts = run.transfer_count_series("demand", "water")
    ...
    assert set(name for name in dir(run) if not name.startswith("_")) == {
        ...
        "quantum",
        "replay_against",
        "transfer_count_series",
        "transfer_series",
    }
    assert isinstance(result, incidence.PresenceSeries)
    assert isinstance(counts, incidence.PresenceCountSeries)
    assert counts.presence == result.presence
