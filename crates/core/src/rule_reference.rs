...
fn parse_identity(
    ...
    let Some(first) = value.chars().next() else {
        return Err(RuleReferenceIdentityError::Empty { kind });
    };
    if !first.is_ascii_lowercase() {
        return Err(RuleReferenceIdentityError::InvalidStart {
            kind,
            value: value.to_owned(),
        });
    }
    for (byte_index, character) in value.char_indices().skip(1) {
        if !(character.is_ascii_lowercase()
            || character.is_ascii_digit()
            || character == '-'
            || character == '_')
        {
            return Err(RuleReferenceIdentityError::InvalidCharacter {
                kind,
                value: value.to_owned(),
                byte_index,
                character,
