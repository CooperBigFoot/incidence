...
use incidence_core::identity::{CompartmentId, IdentityError, IdentityKind};
...
fn identifiers_enforce_the_exact_shared_grammar() {
    macro_rules! assert_conversion_matrix {
        ($type:ty, $kind:ident, $accepted:literal, $invalid_start:literal, $invalid_tail:literal, $byte_index:literal) => {{
            let from_str = $accepted.parse::<$type>();
            assert_eq!(from_str, <$type>::parse($accepted));
            match from_str {
                Ok(identity) => assert_eq!(identity.as_str(), $accepted),
                Err(error) => panic!("accepted FromStr identity was rejected: {error}"),
            }
            let try_from = <$type>::try_from($accepted);
            assert_eq!(try_from, <$type>::parse($accepted));
            match try_from {
                Ok(identity) => assert_eq!(identity.as_str(), $accepted),
                Err(error) => panic!("accepted TryFrom identity was rejected: {error}"),
            }

            let empty_error = Err(RuleReferenceIdentityError::Empty {
                kind: RuleReferenceKind::$kind,
            });
            assert_eq!("".parse::<$type>(), <$type>::parse(""));
            assert_eq!("".parse::<$type>(), empty_error);
            assert_eq!(<$type>::try_from(""), <$type>::parse(""));
            assert_eq!(
                <$type>::try_from(""),
            ...
            let invalid_start_error = Err(RuleReferenceIdentityError::InvalidStart {
                kind: RuleReferenceKind::$kind,
                value: $invalid_start.to_owned(),
            });
            assert_eq!(
                $invalid_start.parse::<$type>(),
                <$type>::parse($invalid_start)
            );
            assert_eq!($invalid_start.parse::<$type>(), invalid_start_error);
            assert_eq!(
                <$type>::try_from($invalid_start),
                <$type>::parse($invalid_start)
            );
            assert_eq!(
                <$type>::try_from($invalid_start),
            ...
            let invalid_tail_error = Err(RuleReferenceIdentityError::InvalidCharacter {
                kind: RuleReferenceKind::$kind,
                value: $invalid_tail.to_owned(),
                byte_index: $byte_index,
                character: '.',
            });
            assert_eq!(
                $invalid_tail.parse::<$type>(),
                <$type>::parse($invalid_tail)
            );
            assert_eq!($invalid_tail.parse::<$type>(), invalid_tail_error);
            assert_eq!(
                <$type>::try_from($invalid_tail),
                <$type>::parse($invalid_tail)
            );
            assert_eq!(
                <$type>::try_from($invalid_tail),
    ...
    assert_conversion_matrix!(InputId, Input, "input-a", "Input-a", "input.a", 5);
    ...
    assert_conversion_matrix!(ForcingId, Forcing, "forcing-a", "Forcing-a", "forcing.a", 7);
    assert_conversion_matrix!(
        ProjectionId,
        Projection,
        "projection-a",
    ...
    assert_conversion_matrix!(TableId, Table, "table-a", "Table-a", "table.a", 5);
    assert_conversion_matrix!(
        TransferBranchId,
        TransferBranch,
        "branch-a",
    ...
    assert_eq!(
        InputId::parse("Input-a"),
        Err(RuleReferenceIdentityError::InvalidStart {
            kind: RuleReferenceKind::Input,
            value: "Input-a".to_owned(),
        })
    );
    assert_eq!(
        TableId::parse("table-A"),
    ...
    assert_eq!(
        ForcingId::parse("forcing.a"),
        Err(RuleReferenceIdentityError::InvalidCharacter {
            kind: RuleReferenceKind::Forcing,
            value: "forcing.a".to_owned(),
            byte_index: 7,
            character: '.',
        })
    );
    ...
    for byte in 0_u8..=0x7f {
        ...
        if byte.is_ascii_lowercase() {
            match InputId::parse(&candidate) {
                Ok(identity) => assert_eq!(identity.as_str(), candidate),
                Err(error) => panic!("valid input identity was rejected: {error}"),
            }
            match CompartmentId::parse(&candidate) {
                Ok(identity) => assert_eq!(identity.as_str(), candidate),
                Err(error) => panic!("valid compartment identity was rejected: {error}"),
            }
        } else {
            assert_eq!(
                InputId::parse(&candidate),
                Err(RuleReferenceIdentityError::InvalidStart {
                    kind: RuleReferenceKind::Input,
    ...
    for byte in 0_u8..=0x7f {
        ...
        let candidate = format!("a{character}");
...
fn truth_depth_128() -> RuleExpr {
    let mut tree = literal(1.25);
    for index in 1..(MAX_RULE_EXPR_DEPTH - 1) {
        tree = if index % 2 == 0 {
            add(tree, literal(2.5))
        } else {
            subtract(tree, literal(3.75))
    ...
    match RuleExpr::comparison(ScalarComparison::GreaterThan, tree, literal(2.5)) {
        Ok(value) => {
            assert_eq!(value.depth(), MAX_RULE_EXPR_DEPTH);
            value
        }
        Err(error) => panic!("truth depth fixture failed: {error}"),
    }
}
...
fn depth_uses_the_deepest_child_in_every_operand_position() {
    ...
    let too_deep = Err(RuleExprError::ExpressionTooDeep {
        maximum: MAX_RULE_EXPR_DEPTH,
        attempted: MAX_RULE_EXPR_DEPTH + 1,
    });
    assert_eq!(RuleExpr::add(deep.clone(), literal(2.5)), too_deep);
    assert_eq!(RuleExpr::add(literal(1.25), deep.clone()), too_deep);
    assert_eq!(RuleExpr::subtract(deep.clone(), literal(2.5)), too_deep);
    assert_eq!(RuleExpr::subtract(literal(1.25), deep.clone()), too_deep);
    assert_eq!(RuleExpr::multiply(deep.clone(), literal(2.5)), too_deep);
    assert_eq!(RuleExpr::multiply(literal(1.25), deep.clone()), too_deep);
    assert_eq!(RuleExpr::divide(deep.clone(), literal(2.5)), too_deep);
    assert_eq!(RuleExpr::divide(literal(1.25), deep.clone()), too_deep);
    assert_eq!(RuleExpr::minimum(deep.clone(), literal(2.5)), too_deep);
    assert_eq!(RuleExpr::minimum(literal(1.25), deep.clone()), too_deep);
    assert_eq!(RuleExpr::maximum(deep.clone(), literal(2.5)), too_deep);
    assert_eq!(RuleExpr::maximum(literal(1.25), deep.clone()), too_deep);
    assert_eq!(
        RuleExpr::clamp(deep.clone(), literal(1.25), literal(2.5)),
        too_deep
    );
    assert_eq!(
    ...
    assert_eq!(
        RuleExpr::clamp(literal(1.25), literal(2.5), deep.clone()),
        too_deep
    );
    assert_eq!(
        RuleExpr::comparison(ScalarComparison::GreaterThan, deep.clone(), literal(2.5)),
        too_deep
    );
    assert_eq!(
        RuleExpr::comparison(ScalarComparison::GreaterThan, literal(1.25), deep.clone()),
        too_deep
    );
    let condition =
    ...
    assert_eq!(
        RuleExpr::select(truth_depth_128(), literal(1.25), literal(2.5)),
        too_deep
    );
    assert_eq!(
        RuleExpr::select(condition.clone(), deep.clone(), literal(2.5)),
        too_deep
    );
    assert_eq!(
        RuleExpr::select(condition, literal(1.25), deep.clone()),
        too_deep
    );
    assert_eq!(
        RuleExpr::interpolated_table(InterpolatedTableRef::new(table_id("table-a")), deep),
        too_deep
    );
}
...
#[test]
fn partition_branch_identity_order_is_canonical_when_fraction_order_disagrees() {
    const JSON: &str = r#"{"rule_ir_version":"v1","numerical_semantics_version":"v1","partition":{"kind":"fixed_fraction_split","retained_fraction":0.0,"branches":[{"branch":"branch-a","fraction":0.75},{"branch":"branch-b","fraction":0.25}]}}"#;
    const HEX: &str = "494e43440001001700010001020000000000000000000000000000000200000000000000086272616e63682d613fe800000000000000000000000000086272616e63682d623fd0000000000000";
    let identity_order = match split(0.0, vec![("branch-a", 0.75), ("branch-b", 0.25)]) {
        Ok(value) => value,
        Err(error) => panic!("identity-order split failed: {error}"),
    };
    let reverse_order = match split(0.0, vec![("branch-b", 0.25), ("branch-a", 0.75)]) {
        Ok(value) => value,
        Err(error) => panic!("reverse-order split failed: {error}"),
    };

    assert_eq!(identity_order, reverse_order);
    for expression in [&identity_order, &reverse_order] {
        assert_eq!(json(expression), JSON);
        let bytes = match CanonicalEncodingVersion::V1.encode(expression) {
            Ok(bytes) => bytes,
            Err(error) => panic!("partition must encode: {error}"),
        };
        assert_eq!(hex(&bytes), HEX);
    }
}

#[test]
fn fixed_split_rejects_non_adjacent_duplicate_identity_with_distinct_fractions() {
    assert_eq!(
        split(
            0.0,
            vec![("branch-a", 0.125), ("branch-b", 0.25), ("branch-a", 0.625),]
        ),
        Err(PartitionExprError::DuplicateBranch {
            branch: branch_id("branch-a"),
        })
