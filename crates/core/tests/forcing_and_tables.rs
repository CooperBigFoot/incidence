...
fn forcing_series_serde_rejects_every_malformed_wire() {
    for wire in [
        ...
        r#"{"id":"forcing-a","horizon":{"first":3,"last":6,"opaque":true},"values":[1.25,0.0,-3.5,2.75]}"#,
...
fn forcing_series_canonical_encoding_is_exact_and_mutation_sensitive() {
    ...
    assert_eq!(
        base_bytes,
        decode_hex(
            "494e43440001001a0000000000000009666f7263696e672d610000000000000003000000000000000600000000000000043ff40000000000000000000000000000c00c0000000000004006000000000000"
        )
    );
    ...
    assert_eq!(
        mutated_bytes,
        decode_hex(
            "494e43440001001a0000000000000009666f7263696e672d610000000000000003000000000000000600000000000000043ff40000000000010000000000000000c00c0000000000004006000000000000"
        )
    );
...
fn interpolation_table_rejects_invalid_shape_and_non_finite_coordinates() {
    ...
    assert_eq!(
        ...
        Err(InterpolationTableError::TooFewPoints { points: 0 })
    );
...
fn interpolation_table_rejects_first_non_increasing_pair_after_zero_normalization() {
    for (xs, expected) in [
        ...
        (
            vec![-2.5, -2.5, 1.25, 1.25],
            InterpolationTableError::AbscissaeNotStrictlyIncreasing {
                left_index: 0,
                right_index: 1,
                left_bits: 0xc004_0000_0000_0000,
                right_bits: 0xc004_0000_0000_0000,
            },
        ),
    ] {
...
fn interpolation_table_serde_rejects_every_malformed_wire() {
    for wire in [
        ...
        r#"{"id":"table-a","numerical_semantics_version":"v1","boundary_policy":"reject","abscissae":[],"ordinates":[]}"#,
...
fn interpolation_table_canonical_encodings_are_exact_and_mutation_sensitive() {
    let fixtures = [
        (
            InterpolationBoundaryPolicy::Reject,
            0x00,
            "494e43440001001b00000000000000077461626c652d610001000000000000000003c00400000000000040248000000000003ff4000000000000400c000000000000401a000000000000bffc000000000000",
        ),
        (
            InterpolationBoundaryPolicy::ClampToEndpoint,
            0x01,
            "494e43440001001b00000000000000077461626c652d610001010000000000000003c00400000000000040248000000000003ff4000000000000400c000000000000401a000000000000bffc000000000000",
        ),
        (
            InterpolationBoundaryPolicy::LinearExtrapolate,
            0x02,
            "494e43440001001b00000000000000077461626c652d610001020000000000000003c00400000000000040248000000000003ff4000000000000400c000000000000401a000000000000bffc000000000000",
        ),
    ];
    ...
    assert_eq!(
        mutated_bytes,
        decode_hex(
            "494e43440001001b00000000000000077461626c652d610001010000000000000003c00400000000000040248000000000003ff4000000000000400c000000000000401a000000000000bffc000000000001"
        )
    );
...
#[test]
fn interpolation_table_normalizes_negative_zero_ordinate() {
    let table = match InterpolationTable::new(
        table_id("table-a"),
        NumericalSemanticsVersion::V1,
        InterpolationBoundaryPolicy::Reject,
        ...
        Ok(table) => table,
        Err(error) => panic!("failed to construct negative-zero ordinate witness: {error}"),
    };
    assert_eq!(
        table
            .points()
            .map(|(_, ordinate)| ordinate.to_bits())
            .collect::<Vec<_>>(),
        [0, 0x400c_0000_0000_0000, 0]
    );
    assert_eq!(
        serde_json::to_string(&table).unwrap(),
        r#"{"id":"table-a","numerical_semantics_version":"v1","boundary_policy":"reject","abscissae":[-2.5,1.25,6.5],"ordinates":[0.0,3.5,0.0]}"#
    );
}

#[test]
fn interpolation_table_normalizes_negative_zero_at_non_first_abscissa() {
    let table = match InterpolationTable::new(
        table_id("table-a"),
        NumericalSemanticsVersion::V1,
        InterpolationBoundaryPolicy::Reject,
        ...
        Ok(table) => table,
        Err(error) => {
            panic!("failed to construct non-first negative-zero abscissa witness: {error}")
        }
    };
    assert_eq!(
        table
            .points()
            .map(|(abscissa, _)| abscissa.to_bits())
            .collect::<Vec<_>>(),
        [0xc004_0000_0000_0000, 0, 0x401a_0000_0000_0000]
    );
    assert_eq!(
        serde_json::to_string(&table).unwrap(),
        r#"{"id":"table-a","numerical_semantics_version":"v1","boundary_policy":"reject","abscissae":[-2.5,0.0,6.5],"ordinates":[10.25,3.5,-1.75]}"#
    );
}

#[test]
fn forcing_series_normalizes_negative_zero_at_non_first_offsets() {
    let series = forcing(3, 6, vec![1.25, -0.0, -3.5, -0.0]);
    assert_eq!(
        series.values().map(f64::to_bits).collect::<Vec<_>>(),
        [0x3ff4_0000_0000_0000, 0, 0xc00c_0000_0000_0000, 0]
    );
    for timestep in [4, 6] {
        assert_eq!(
            series.value_at(TimestepIndex::new(timestep)),
        ...
        match series.value_at(TimestepIndex::new(timestep)) {
            ValueState::Present(value) => assert_eq!(value.to_bits(), 0),
            ValueState::Absent => {
                panic!("in-horizon negative-zero witness was absent at {timestep}")
            }
            ValueState::NotModelled => panic!("forcing series returned not-modelled at {timestep}"),
        }
    }
    assert_eq!(
        serde_json::to_string(&series).unwrap(),
        r#"{"id":"forcing-a","horizon":{"first":3,"last":6},"values":[1.25,0.0,-3.5,0.0]}"#
    );
}

#[test]
fn interpolation_table_normalizes_negative_zero_at_every_ordinate_index() {
    let table = match InterpolationTable::new(
        table_id("table-a"),
        NumericalSemanticsVersion::V1,
        InterpolationBoundaryPolicy::Reject,
        ...
        Ok(table) => table,
        Err(error) => {
            panic!("failed to construct exhaustive negative-zero ordinate witness: {error}")
        }
    };
    assert_eq!(
        table
    ...
    assert_eq!(
        serde_json::to_string(&table).unwrap(),
        r#"{"id":"table-a","numerical_semantics_version":"v1","boundary_policy":"reject","abscissae":[-2.5,1.25,6.5],"ordinates":[0.0,0.0,0.0]}"#
    );
}

#[test]
fn interpolation_table_normalizes_negative_zero_at_last_abscissa() {
    let table = match InterpolationTable::new(
        table_id("table-a"),
        NumericalSemanticsVersion::V1,
        InterpolationBoundaryPolicy::Reject,
        ...
        Ok(table) => table,
        Err(error) => {
            panic!("failed to construct last-index negative-zero abscissa witness: {error}")
        }
    };
    assert_eq!(
        table
            ...
            .map(|(abscissa, _)| abscissa.to_bits())
            .collect::<Vec<_>>(),
        [0xc004_0000_0000_0000, 0xbff4_0000_0000_0000, 0]
    );
    assert_eq!(
        serde_json::to_string(&table).unwrap(),
        r#"{"id":"table-a","numerical_semantics_version":"v1","boundary_policy":"reject","abscissae":[-2.5,-1.25,0.0],"ordinates":[10.25,3.5,-1.75]}"#
    );
}

#[test]
fn forcing_series_normalizes_negative_zero_at_every_offset() {
    let series = forcing(3, 6, vec![1.25, -0.0, -0.0, -0.0]);
    assert_eq!(
        series.values().map(f64::to_bits).collect::<Vec<_>>(),
        [0x3ff4_0000_0000_0000, 0, 0, 0]
    );
    for timestep in [4, 5, 6] {
        ...
        match series.value_at(TimestepIndex::new(timestep)) {
            ValueState::Present(value) => assert_eq!(value.to_bits(), 0),
            ValueState::Absent => {
                panic!("in-horizon exhaustive negative-zero witness was absent at {timestep}")
            }
            ValueState::NotModelled => panic!("forcing series returned not-modelled at {timestep}"),
        }
    }
    assert_eq!(
        serde_json::to_string(&series).unwrap(),
        r#"{"id":"forcing-a","horizon":{"first":3,"last":6},"values":[1.25,0.0,0.0,0.0]}"#
    );
}
